...
- Your Keyring app origin as `baseUrl` (e.g. `https://app.example.com`).
...
const keyring = new Keyring({
  ...
  baseUrl: process.env.KEYRING_URL!, // your Keyring origin
});
...
The secret key needs the `check` scope to check, `grants.write` to grant/revoke, and `subject_tokens.write` to mint browser tokens.
</Step>
