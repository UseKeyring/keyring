...
// While the waitlist is on, the footer drops every link into the app and
// points at the signup page instead — no route into /auth or /dashboard.
export function SiteFooter({ waitlist = false }: { waitlist?: boolean }) {
  const sections = waitlist
    ? [
        {
          title: "Waitlist",
          links: [{ title: "Join the waitlist", href: "/waitlist" }],
        },
      ]
    : SECTIONS;
  return (
              ...
              <div className="flex flex-col gap-2">
                {s.links.map((l) =>
                  l.href.startsWith("/#") || l.href.startsWith("#") ? (
                    ...
                    <Link
                      key={l.title}
                      href={l.href as "/dashboard" | "/dashboard/account" | "/auth" | "/pricing" | "/waitlist"}
                      className="type-label w-fit text-ink-navy transition-colors hover:text-ink"
                    >
                      {l.title}
                    </Link>
