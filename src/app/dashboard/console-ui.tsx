...
export function OverviewSetupSteps({
  ...
  const aiPromptText = [
    ...
    `- Base URL: /api/v1 (Authorization: Bearer <YOUR_KEYRING_API_KEY>)`,
    `- Check Permission: GET /api/v1/check?subject=<USER_ID>&permission=<ACTION_SLUG>`,
    `- Grant Role to User: POST /api/v1/grants { "role": "<ROLE_SLUG>", "subject": "<USER_ID>" }`,
    `- Revoke Role from User: DELETE /api/v1/grants { "role": "<ROLE_SLUG>", "subject": "<USER_ID>" }`,
    ...
    `- Implement backend middleware calling /api/v1/check to enforce these permissions on protected endpoints.`,
  ].join("\n");
